...
class TestCrisisDetectorBasic:
    ...
    def test_process_signal_with_spike(self):
        ...
        # Create signal with a spike (crisis) - use fixed seed for reproducibility
        np.random.seed(42)
        n_points = 300
        signal = np.random.randn(n_points) * 0.1  # Very low noise

        # Add a clear spike in the middle
        spike_start = 140
        spike_end = 160
        signal[spike_start:spike_end] += 15  # Very large spike
        ...
        assert np.any(
            results["crisis_regions"]
        ), "Spike should be detected as crisis region"
        assert np.any(results["anomalies"]), "Spike should be detected as anomaly"

        # Max crisis score should be high
        assert (
            np.max(results["crisis_score"]) > 0.5
        ), "Max crisis score should be elevated"
