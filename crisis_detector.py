...
class CrisisDetector:
    ...
    def process_signal(
        ...
        rolling_mean = pd.Series(signal_values).rolling(
            window=self.window_size, center=True
        ).mean().values
        rolling_std = pd.Series(signal_values).rolling(
            window=self.window_size, center=True
        ).std().values
        ...
        z_scores[valid_std] = (
            (signal_values[valid_std] - rolling_mean[valid_std]) / rolling_std[valid_std]
        )
        
        # Calculate volatility (rate of change)
        volatility = np.abs(np.gradient(signal_values))
        volatility_norm = (volatility - np.nanmean(volatility)) / (np.nanstd(volatility) + 1e-10)
    ...
    def _calculate_metrics(
        ...
        metrics = {
            ...
            'crisis_ratio': n_crisis_points / n_total_points if n_total_points > 0 else 0,
    ...
    def plot_analysis(
        ...
        if np.any(crisis_regions):
            axes[0].fill_between(
                timestamps, signal.min(), signal.max(),
                where=crisis_regions, alpha=0.3, color='red',
